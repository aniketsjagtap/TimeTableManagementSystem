...
$sql = "SELECT * from "._tbl('faculty');
$res = db()->query($sql);
 $result = $res->all_rows_keyed();
//print_r($res->next_row_keyed());
echo "<div class=\"datatable_container\" > <table id=\"faculty_table\" class=\"admin_table display\" >";
echo "<thead >";
        /*echo "<td>";
        echo "ID";
        echo "</td>";*/
...
foreach($result as $rs)
{
        echo "<tr>";
               /* echo"<td>";
                        echo $rs['id'];
                echo "</td>";*/
		 $html_name = htmlspecialchars($rs['name']);
            	 $href = multisite('edit_faculty.php?faculty=' . $rs['id']);
            // We insert an invisible span containing the sort key so that the rooms will
            // be sorted properly
            	echo "<td><div>" .
                 "<span>" . htmlspecialchars($rs['sort_key']) . "</span>" .
...
$sql = "SELECT * from "._tbl('division');
$res = db()->query($sql);
 $result = $res->all_rows_keyed();
//print_r($res->next_row_keyed());
echo "<div class=\"datatable_container\" > <table  id=\"division_table\" class=\"admin_table display\" >";
echo "<thead >";
      /*  echo "<td>";
        echo "ID";
        echo "</td>";*/
...
foreach($result as $rs)
{
        echo "<tr>";
               /* echo"<td>";
                ...
                echo "</td>";*/
		$html_name = htmlspecialchars($rs['name']);
                 $href = multisite('edit_division.php?division=' . $rs['id']);
            // We insert an invisible span containing the sort key so that the division will
            // be sorted properly
                echo "<td><div>" .
                 "<span>" . htmlspecialchars($rs['sort_key']) . "</span>" .
                 "<a title=\"$html_name\" href=\"" . htmlspecialchars($href) . "\">$html_name</a>" .
...
$sql = "SELECT * from "._tbl('course');
$res = db()->query($sql);
 $result = $res->all_rows_keyed();
//print_r($res->next_row_keyed());
echo "<div class=\"datatable_container\" > <table  id=\"course_table\" class=\"admin_table display\" >";
echo "<thead >";
       /* echo "<td>";
        echo "ID";
        echo "</td>";*/
...
foreach($result as $rs)
{
        echo "<tr>";
               /* echo"<td>";
                ...
                echo "</td>";*/
		$html_name = htmlspecialchars($rs['name']);
                 $href = multisite('edit_course.php?course=' . $rs['id']);
            // We insert an invisible span containing the sort key so that the rooms will
            // be sorted properly
                echo "<td><div>" .
                 "<span>" . htmlspecialchars($rs['sort_key']) . "</span>" .
...
$form = new Form();
  ...
  $form->addHiddenInput('type', 'division');

  // Now the visible fields
  $fieldset = new ElementFieldset();
  $fieldset->addLegend(get_vocab('adddivision'));
...
*/
  ...
  $field->setControlAttributes(array('value' => get_vocab('adddivision'),
                                     'class' => 'submit'));
  $fieldset->addElement($field);
...
$form = new Form();
  ...
  $attributes = array('id'     => 'add_faculty',
                      'class'  => 'form_admin standard',
                      'action' => multisite('add.php'),
                      'method' => 'post');
  ...
  // Hidden field for the type of operation
  $form->addHiddenInput('type', 'faculty');

  // Now the visible fields
  $fieldset = new ElementFieldset();
  $fieldset->addLegend(get_vocab('addfaculty'));
...
*/
  ...
  // The submit button
  $field = new FieldInputSubmit();
  $field->setControlAttributes(array('value' => get_vocab('addfaculty'),
                                     'class' => 'submit'));
  $fieldset->addElement($field);
...
function addCourseForm(){
  ...
  $attributes = array('id'     => 'add_course',
                      'class'  => 'form_admin standard',
                      'action' => multisite('add.php'),
                      'method' => 'post');
  ...
  // Hidden field for the type of operation
  $form->addHiddenInput('type', 'course');

  // Now the visible fields
  $fieldset = new ElementFieldset();
  $fieldset->addLegend(get_vocab('addcourse'));
  ...
  // The submit button
  $field = new FieldInputSubmit();
  $field->setControlAttributes(array('value' => get_vocab('addcourse'),
                                     'class' => 'submit'));
  $fieldset->addElement($field);

...
// $standard_fields['entry'][23]='faculty';
// echo "<br> Standard_fields: ";
// print_r($standard_fields['entry']);
// echo "<br>";
...
//echo "<br> custom_fields_map:";
//print_r($custom_fields_map);
...
//print_r($res);
  ...
  $label->setText(get_vocab('faculty'));
...
function get_field_division($disabled = false)
{
$sql = "SELECT id,name FROM "._tbl('division');
$res = db()->query_array1($sql);
  $element = new ElementDiv();
  
  $label = new ElementLabel();
  $label->setText(get_vocab('division'));
     //   ->setAttribute('class', 'no_suffix');
  $element -> addElement($label);
...
$res = db()->query_array($sql);
  ...
  $label = new ElementLabel();
  $label->setText(get_vocab('course'));
      //  ->setAttribute('class', 'no_suffix');
  $element -> addElement($label);
...
/*
$faculties = array();
$sql = "SELECT id, name, email, description 
	FROM "._tbl('faculty')." WHERE
...
{
 //       print_r($row);
  // Only use faculty which are visible and for which the user has write access
  if (getWritable($create_by, $row['id']) && is_visible($row['id']))
  {
    $faculties[$row['id']][$row['id']] = $row['name'];
  }
}
print_r($faculties);
*/
...
//      $fieldset->addElement(get_field_faculty());
      break;
	...
	break;
     ...
     case 'division';
        $fieldset->addElement(get_field_division());
        break;
     
     case 'course';
        $fieldset->addElement(get_field_course());
        break;
